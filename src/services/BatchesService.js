...
async function failPackageByResi(resi) {
  return db.transaction(async (trx) => {
    // Ambil package
    const pkg = await trx("packages").where({ resi }).first();
    if (!pkg) throw new NotFoundError(`Package dengan resi ${resi} tidak ditemukan`);
    ...
    await trx("batches_pesawat").where({ id: batchId }).update({
    ...
    // Hitung berat & harga baru
    const { weightUsed, price } = calculatePackageDetails({ ...pkg, kode: newKode, via: "Kapal" });

    // Update package
    await trx("packages").where({ id: pkg.id }).update({
      kode: newKode,
      via: "Kapal",
...
// Ambil semua paket failed xray
async function getAllFailedXray() {
  return db("failed_xray")
